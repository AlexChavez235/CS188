...
class Graph(object):
    ...
    def backprop(self):
        ...
        for node in self.get_nodes()[::-1]:
            outputs = node.backward(self.get_inputs(node), self.get_gradient(node))
            i = 0
            for parent in node.get_parents():
                self.gradients[parent] += outputs[i]
                i = i+1
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        #input contains a list of the output values of the node's parents where each value of the list is a numpy array
        #the list is the same length and same order as self.get_parents
        x = inputs[0]
        z = np.zeros(x.shape)
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        np.reshape(b, a.shape)
        new = np.subtract(a,b)
        const = gradient/new.size
        return [const*(new), const*(-new)]
        #return [np.dot(new, -1*b), np.dot(new, a)]
