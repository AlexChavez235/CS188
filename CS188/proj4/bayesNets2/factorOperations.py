...
def joinFactors(factors):
    ...
    #make a new factor with these variables
    newFactor = Factor(unconditionedVars, conditionedVars, factors[0].variableDomainsDict())
    for assignment in newFactor.getAllPossibleAssignmentDicts():
        prob = 1
        for factor in factors:
            prob = prob*factor.getProbability(assignment) #can take in more variables than in the factor
...
def normalize(factor):
    ...
    for var in unconditionedVars.copy():
        if len(dict[var]) == 1:
