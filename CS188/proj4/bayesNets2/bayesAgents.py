...
def constructBayesNet(gameState):
    ...
    variables = [X_POS_VAR, Y_POS_VAR] + HOUSE_VARS + obsVars #list with all variables
...
def fillObsCPT(bayesNet, gameState):
    ...
    #coordinates of each possible house location
    "*** YOUR CODE HERE ***"
    d = {bottomLeftPos: BOTTOM_LEFT_VAL, topLeftPos: TOP_LEFT_VAL, bottomRightPos: BOTTOM_RIGHT_VAL, topRightPos: TOP_RIGHT_VAL}
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos): #gives us pos around house walls
            obsVar = OBS_VAR_TEMPLATE % obsPos
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                if d[housePos] == assignment[FOOD_HOUSE_VAR]:
                    if assignment[obsVar] == RED_OBS_VAL:
                        ...
                        prob = 0
                elif d[housePos] == assignment[GHOST_HOUSE_VAR]:
                    if assignment[obsVar] == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif assignment[obsVar] == BLUE_OBS_VAL:
    ...
    for foodHouse in HOUSE_VARS:
        ...
        for assignment in factor.getAllPossibleAssignmentDicts():
            if factor.getProbability(assignment) > prob:
                ...
                location = assignment[FOOD_HOUSE_VAR]
        dict[foodHouse] = location
    return dict
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        leftExpectedValue = WON_GAME_REWARD*prob1 + GHOST_COLLISION_REWARD*prob2
        rightExpectedValue = WON_GAME_REWARD*prob2 + GHOST_COLLISION_REWARD*prob1
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for pair in self.getExplorationProbsAndOutcomes(evidence):
            maxAction = max(self.computeEnterValues(pair[1], enterEliminationOrder))
            expectedValue = expectedValue + pair[0]*maxAction
