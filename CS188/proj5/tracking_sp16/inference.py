...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #newPosDist[p] is prob that ghost is at posiiton p at time t+1, given that the ghost is at position oldPos at time t
        oldBeliefs = self.beliefs.copy()
        self.beliefs = DiscreteDistribution()
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for pos in self.allPositions:
                self.beliefs[pos] += newPosDist[pos]*oldBeliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.particles:
            weights[pos] = weights[pos] + self.getObservationProb(observation, pacmanPosition, pos, jailPosition)
        if weights.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = []
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        while counter < self.numParticles:
            self.particles.append(permutations[counter%len(permutations)])
            counter = counter+1
